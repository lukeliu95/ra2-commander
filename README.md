...
### 三次进化：match-004、match-005、match-006 输了之后发生了什么
...
这些经验已经作为初始知识写进 `assets/seed/`：方案 v000→v006、对手档案 `ai-easy`（盟军两种打法、苏军、古巴四个小节）、战术库 T-001～T-026、六局的完整记录，外加一份跨局的单位数据库 `knowledge/units.json`。下一局默认使用 v006。
...
- 已测试环境：gonghui.k0s.cn 客户端 v0.87.0，单机模式 → 遭遇战，AI-简单，地图"岛屿之战 (2)"。我方打过盟军（法国）和苏军；对手遇到过盟军、苏军、Confederation（古巴，第三阵营）三种，出生点会互换，双方也可能是同阵营镜像局（match-006 双方都是盟军）。
...
assets/seed/                初始经验：方案 v000–v006、对手档案、战术库、六局记录（含每局全部复盘产物）
