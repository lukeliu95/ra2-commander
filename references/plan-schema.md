...
- 攻城：进攻中附近没有敌军时，直接集火建筑。**目标顺序按"离结束比赛有多近"排，不按"多烦人"排**：建造厂 > **生产建筑（兵营/战车工厂/空指部/雷达/实验室）** > 电厂 > 防御 > 矿厂，核电站排最后。
...
- **清场模式**：转入进攻后，只要敌方**野战部队总价值 < 1500**，就认定"没有任何力量能惩罚我们的全力投入"，于是：① 全军加入攻势（不再要求单位在集结点 10 格内才算入 `A.ids`）；② 进攻部队的自动撤退下限降到一个**绝对的 0.14**（`min(retreatRatio, 0.14)`）；③ 攻城的阻挡容忍度从 25% 提到 60%。日志记一条 `[attack] 敌方野战力量仅 N：进入清场模式`。
