...
| `defendRetreatRatio` / `defendMinValue` | 防守交战时，部队价值跌到交战开始时的这个比例、**或者**绝对值跌破 `defendMinValue`，就撤回基地，不再继续原地对射（T-022 的加权只影响判断，真正"打不过就撤"靠这两个字段——match-007 之前防守分支没有任何退出条件，会打到全灭；match-008 发现残部基数很小时，比例可能在一次采样间隔里直接从远高于阈值跳到 0%，绝对值下限是补丁，不是完整修复） | 0.4 / 800 |
